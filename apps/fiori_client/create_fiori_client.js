...
function updateConfigXML() {
    ...
    var getPluginInfo = function(plugin) {
        if (typeof plugin === 'object') {
            ...
            var index = plugin.lastIndexOf('@')
            if (index > 0)
            {
                var parts = plugin.split('@');
                return {
                    'id': plugin.substring(0, index),
                    'version': plugin.substring(index + 1)
                }
            }
            else {
                return {
    ...
    if (config.crosswalkEnabled === true) {
        if (!config.cordovaPluginIncludes) {
            config.cordovaPluginIncludes = [];
        }

        config.cordovaPluginIncludes.push('cordova-plugin-crosswalk-webview');
        config.cordovaPluginIncludes.push('kapsel-plugin-inappbrowser-xwalk');
    }

    // Add any extra plugins specified in the config
    if (config.cordovaPluginIncludes && config.cordovaPluginIncludes.length > 0) {
    	// With iOS Cordova 6.0.0 and above version, the 'cordova-plugin-wkwebview-engine' plugin wasn't mandatory. So, just ignore it to avoid the build error.
        var index = config.cordovaPluginIncludes.indexOf('cordova-plugin-wkwebview-engine');
        if (index >= 0) {
            config.cordovaPluginIncludes.splice(index, 1);
        }
