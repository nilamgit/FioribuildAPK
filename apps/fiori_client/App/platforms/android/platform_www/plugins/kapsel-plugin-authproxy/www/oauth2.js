    ...
    if (device.platform.toLowerCase() == "ios" && currentConfig.useExternalBrowser) {
        ...
        var loadErrorListener = function (event) {
            if (cordova.require("cordova/platform").id == "ios" ){
                ...
                // policy change. 1 is for page reload before previous loading is finished. Neither should be fatal.
                if (event && (event.code === -999 || event.code === 102 || event.code === 1)) {
        ...
        var iabOption = "location=no,toolbar=yes,hardwareback=no";
        if (enableClearCache) {
            iabOption += ",clearcache=yes";
        }
        utils.log("Opening InAppBrowser for OAuth2 authorization. iabOption = '" + iabOption + "'");
        windowRef = cordova.InAppBrowser.open(url, '_blank', iabOption);
...
/**
 * set to true to have the inappbrowser's cookie cache cleared before starting
 * the OAuth2 authorization process.
 * @param {boolean} value  A boolean value to be set to enable or diable clear cache. Default is false.
 * @memberof sap.AuthProxy.OAuth2
 * @static
 * @example
 * // Configure AuthProxy to clear cookie cache before opening the inappbrowser
 * // for OAuth2 authorization.
 * sap.AuthProxy.OAuth2.setEnableClearCache(true);
 */
function setEnableClearCache(value) {
    if (typeof value === "boolean") {
        enableClearCache = value;
    } else {
        utils.log("Invalid type for argument 'value' in function setEnableClearCache. Only boolean is accepted.");
    }
}
...
module.exports = {
    ...
    setEnableClearCache: setEnableClearCache
};
