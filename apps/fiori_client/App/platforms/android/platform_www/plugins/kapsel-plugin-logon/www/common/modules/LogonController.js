...
var getConfigFromMobilePlace = function (onGetConfigData, context) {
    ...
    //switch to dev testing mobile place server based on the email domin.
    ...
    } else if (emailDomain.indexOf("next.sapmobileplace.com") != -1 || emailDomain.indexOf("smoketest.sap.com") != -1) {
        ...
        for (var i = 0; i < inputContext.auth.length; i++) {
            if (inputContext.auth[i]["type"] == "oauth2") {
                ...
                if (inputContext.auth[i]["config"]["oauth2.clientID"] == null) {
                    // check if "oauth2.clients" array has item
                    if (inputContext.auth[i]["config"]["oauth2.clients"] && inputContext.auth[i]["config"]["oauth2.clients"].length > 0 && inputContext.auth[i]["config"]["oauth2.clients"][0].clientID != null) {
                        inputContext.auth[i]["config"]["oauth2.clientID"] = inputContext.auth[i]["config"]["oauth2.clients"][0].clientID;
                    } else {
                ...
                if (inputContext.auth[i]["config"]["oauth2.grantType"] == null) {
                    // check if "oauth2.clients" array has item
                    if (inputContext.auth[i]["config"]["oauth2.clients"] && inputContext.auth[i]["config"]["oauth2.clients"].length > 0 && inputContext.auth[i]["config"]["oauth2.clients"][0].grantType != null) {
                        inputContext.auth[i]["config"]["oauth2.grantType"] = inputContext.auth[i]["config"]["oauth2.clients"][0].grantType;
                    } else {
    ...
    if (_providedContext && _providedContext.mobilePlace) {
        function onEnterEmailSubmit(context) {
            ...
            if (emailRegex.test(email)) {
                ...
                // switch to dev testing mobile place servers based on the email domain.
                var emailDomainIndex = email.indexOf("@");
                var emailDomain = email.substring(emailDomainIndex);
                if (emailDomain.indexOf("murray.sapmobileplace.com") != -1) {
                    mobilePlaceHost = "https://portal.murray.sapmobilesecure.com";
...
var pingServer = function (context, successCallback, errorCallback, retry) {
    ...
    if (cordova.require("cordova/platform").id.indexOf("windows") === -1) {
        ...
        // NOTE: authConfig certificate type "LOGON" is not supported on Windows,
        // so assign null to authConfig.
        authConfig = null;
    }
...
var RegistrationFlow = function RegistrationFlow(logonCore, logonView, onCoreResult, onFlowSuccess, onFlowError, onFlowCancel) {
    ...
    var onDenyEulaPsAgreement = function () {
        onFlowError(errorWithDomainCodeDescription("MAFLogon", "1", "User denied EULA and Privacy Statement"));
    }
    ...
    var onRegNewUser = function (context) {
        ...
        /**
         * callback function after user accepted EULA and PS agreement.
         */
        var onAcceptAgreementCallback = function () {
        ...
        // Close inappbrowser first to allow pop-up EULA dialog on main webview.
        logonView.close(function () {
            // pop-up EULA and Privacy Statement
            if (sap.FioriClient) {
                var forRegNewUser = true;
                sap.FioriClient.checkAndOpenEulaPsDialog(forRegNewUser).then(function (oEvent) {

                    if (oEvent.accept) {
                        onAcceptAgreementCallback();
                    } else {
                        // User denied agreement. Cancel reg new user
    ...
    var onUnlockSubmit = function (context) {
        ...
        if (context.multiUser && context.multiUserInfo.currentUser != context.currentSelectedUser.deviceUserId) {
            ...
            if (sap.FioriClient) {
                ...
                // Hide InAppBrowser window first to show the EulaPs popup dialog.
                logonView.hideWindow();
                sap.FioriClient.checkAndOpenEulaPsDialogForMultiUser(context.currentSelectedUser.deviceUserId).then(function (oEvent) {
                    logonView.showWindow();
                    if (oEvent.accept) {
                        logonCore.activateSecureStoreForUser(onCoreResult, onUnlockError, context);
                    } else {
                        // User denied agreement.
                        onDenyEulaPsAgreement();
                    }
                }).catch(function (errMsg) {
                    logonView.showWindow();
                    sap.Logger.Error("Failed to open EULA and Privacy Statement dialog. " + errMsg, "LOGON");
                    onUnlockError(errMsg, context);
                });
            } else {
                logonCore.activateSecureStoreForUser(onCoreResult, onUnlockError, context);
            }
            ...
            if (context.passcodeEnabled) {
                ...
                logonCore.persistRegistration(
                    function (currentContext, state) {
                        ...
                        if (sap.FioriClient && state.multiUser && state.multiUser.currentUser) {
                            sap.FioriClient.saveAgreementForUser(state.multiUser.currentUser).then(function () {
                                continueWith();
                            }).catch(function (err) {
                                console.error(err);
                            });
...
var getConfiguration = function (onsuccess, onerror, type) {
    ...
    if (type == "authentication") {
        ...
        if (device.platform.toLowerCase() === "windows") {
            // Basic type "USER" is needed on windows to pop up basic auth dialog.
            // Certificate type "LOGON" is not supported on windows.
            jsonConfig = '{"basic":[{"type":"logon"},{"type":"user"}]' + samlConfig + '}';
        } else {
            jsonConfig = '{"basic":[{"type":"logon"}],"clientCert":[{"type":"logon"}]' + samlConfig + '}';
        }
