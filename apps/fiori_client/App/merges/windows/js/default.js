...
(function () {
    ...
    var FIORI_HELP_URL = "https://help.sap.com/doc/d55f83e12e4b40779158fbaf08fe0f14/1.11/en-US/index.html";
    ...
    var injectBridgeShims =
        "function addScript(fileName) {" +
            "return new Promise(function(resolve, reject) {" +
                ...
                "try {" +
                    // There is an issue on the FioriLaunchpad: For the newly appended script tag, src will be auto
                    // appended prefix like "/cloud/flp/3.83.2/", which makes the cordova bridge injection failed.
                    // To fix it, we call appendChild before setting src attribute.
                    ...
                    "serverScript.onload = function() {" +
                        "resolve();" +
                    "};" +
                    "console.log ('Script ' + fileName + ' added successfully');" +
                "} catch(e) {" +
                    "console.log ('ERROR: adding script: ' + fileName + ', error: ' + e.message);" +
                    "reject('ERROR: adding script: ' + fileName + ', error: ' + e.message);" +
                "}" +
            "});" +
        "}" +

        "addScript('common-cordova-bridge-server.js').then(function() {" +
            // Following bridges depend on common bridge, so put them in Promise.then to make sure
            // that following scripts are added after 'common-cordova-bridge-server.js' is onloaded.
            // Otherwise, those scripts will be loaded concurrently and result in unexpected error.
    ...
    function afterProcess() {
        ...
        i18nBundle.load(function () {
           ...
           document.addEventListener('fioriinitialized', onFioriInitialized);

            var customEvent = document.createEvent('Event');
            customEvent.initEvent('defaultinitialized', true, true);
            document.dispatchEvent(customEvent);
        });
    }

    function onFioriInitialized() {
        if (protocolUrl != null) {
            sap.FioriClient.handleOpenURL(function () {
            }, protocolUrl);
            protocolUrl = null;
    ...
    function handleChangeFioriClientEvent() {
        ...
        handleEvent('resetSettings', function (event) {
            ...
            if (event.detail && event.detail.args && event.detail.args.skipReload) {
                shouldReload = false;
            }
            if (shouldReload) {
                setTimeout(function () {
