...
var moduleMap = {};
var whiteListedUrlsFromParameter = [];
...
module.exports = {
    ...
    setWhiteList: function (win, fail, args) {
        try {
            var whiteListedUrl = args[0];
            whiteListedUrlsFromParameter.push(whiteListedUrl);
            if (typeof win === "function") {
                win();
            }
        } catch (ex) {
            console.error("Cannot set White List. Caused by: " + ex.toString());
    ...
    getWhiteList: function (win, fail) {
        win(whiteListedUrlsFromParameter);
    },
