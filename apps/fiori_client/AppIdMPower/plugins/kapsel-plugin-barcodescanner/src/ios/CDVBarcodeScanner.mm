...
- (void)scan:(CDVInvokedUrlCommand*)command {
    ...
    //customization for kapsel sdk
    UIViewController* vc = self.viewController;
    if (vc.view.window != [UIApplication sharedApplication].keyWindow){
        //for wkwebview, the privacyscreen or inappbrowser plugin is presented from a different window object
...
- (void)openDialog {
    if (@available(iOS 13.0, *)) {
        [self.viewController setModalInPresentation:true];
    }
...
- (NSString*)formatStringFromMetadata:(AVMetadataMachineReadableCodeObject*)format {
    ...
    if (format.type == AVMetadataObjectTypeInterleaved2of5Code) return @"ITF2/5";
    //end customization
    return @"???";
}
...
- (NSArray*) formatObjectTypes {
    ...
    //kapsel customization
    if (self.formats == nil || [supportedFormats containsObject:@"ITF2/5"]) [formatObjectTypes addObject:
