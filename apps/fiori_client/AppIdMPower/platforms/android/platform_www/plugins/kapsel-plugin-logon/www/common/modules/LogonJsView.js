cordova.define("kapsel-plugin-logon.LogonJsView", function(require, exports, module) {
    ...
    var evalIabEvent = function (event) {
        ...
        if (fragments) {
            if (handleEvent[cordova.require("cordova/platform").id][event.type]) {
                // Use the new way to get information out of the IAB using executeScript
                if (fragments[1] == "EVENTOCCURRED") {
                    ...
                    // On Windows, the invoked script can return only string values, so convert event to a string
                    // using JSON.stringify. Ref:
                    // https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.webview.invokescriptasync
                    var payload = "var event = eventsObject[" + identifier + "];\n"+
                        "delete eventsObject[" + identifier + "];\n"+
                        "JSON.stringify(event);"; // The last line must be the value we want to return to the host main webview.
                    var callback = function(result) {
                        var eventObject = JSON.parse(result[0]);
    ...
    var newScreen = function (path) {
        ...
        // use Function.prototype.bind to pass windowRef as 'this' object to the event listeners. 
    ...
    /**
     * Hide InAppBrowser window. calling this function has no effect if the
     * InAppBrowser window was already hidden.
     */
    function hideWindow() {
        if (windowRef) {
            windowRef.hide();
    ...
    /**
     * Show InAppBrowser window. Calling this function has no effect if the
     * InAppBrowser window was already visible.
     */
    function showWindow() {
        if (windowRef) {
            windowRef.show();
