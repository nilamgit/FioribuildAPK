...
AuthProxy.prototype.generateODataHttpClient = function () {
   var httpClient = {
        request: function (request, success, error) {
            ...
            abortClient = AuthProxy.prototype.sendRequest(request.method || "GET", url, requestHeaders, requestBody, successCB, errorCB, requestUserName, requestPassword, requestTimeout, requestCertificate);
   ...
   var httpClient = {
        request: function (request, success, error) {
            ...
            if (request.authConfig || (!(sap && sap.Logon))) {
                abortClient = AuthProxy.prototype.sendRequest2(request.method || "GET", url, requestHeaders, requestBody, successCB, errorCB, requestTimeout, request.authConfig);
            }
            else{
                var configHandler = function(jsonconfig) {
                    ...
                    abortClient = AuthProxy.prototype.sendRequest2(request.method || "GET", url, requestHeaders, requestBody, successCB, errorCB, requestTimeout, authConfig);
                };
...
//copied from generateODataHttpClient2, generate http client for document service for WK webview on iOS platform
AuthProxy.prototype.generateDocumentServiceHttpClient = function () {
 var httpClient = {
      request: function (request, success, error) {
          var url, requestHeaders, requestBody, statusCode, statusText, responseHeaders;
          var responseBody, requestTimeout
...
//As unlike UIWebView, WKWebview does not allow the xmlhttprequest to send cross domain reqeusts to remote server.
    ...
    } else if (cordova.require("cordova/platform").id == "windows") {
        // On Windows, set authproxy as default OData Http Client.
        // The reason is xmlhttprequest cannot handle basic auth challenge, such as cache credential or loading
        // credential from Logon plugin.
        if ((typeof OData == "undefined") && (typeof jQuery != 'undefined') && jQuery.sap) {
            //if Odata module is not yet loaded, try to load it from UI5 library if available
            jQuery.sap.require("sap.ui.thirdparty.datajs");
        }
        if (typeof OData != "undefined") {
            var datajsHttpClient = OData.defaultHttpClient;
            var authProxyHttpClient = AuthProxyPlugin.generateODataHttpClient2();

            // The kapselHttpClient will delegate SYNC request to the original datajs HttpClient, and delegate ASYNC request to AuthProxy,
            // because AuthProxy doesn't support SYNC mode. SYNC mode is necessary for SAPUI5 OData libary to refresh the x-csrf-token.
            var kapselHttpClient = {
                wrappedHttpClient: datajsHttpClient,
                authProxyHttpClient: authProxyHttpClient,
                request: function (req, success, error) {
                    if (req.async === false) {
                        return this.wrappedHttpClient.request(req, success, error);
                    } else {
                        return this.authProxyHttpClient.request(req, success, error);
                    }
                }
            };
            OData.defaultHttpClient = kapselHttpClient;
        }
    }
