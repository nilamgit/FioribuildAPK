cordova.define("kapsel-plugin-authproxy.saml2", function(require, exports, module) {
...
function authenticate(successCallback, errorCallback) {
    ...
    if (sap && sap.logon) {
        ...
        sap.logon.Core.getState (function (result) {
            if(result.status == "fullRegistered"&&result.hasSecureStore==true&&result.secureStoreOpen==false) {
                utils.log("do nothing in saml2 authenticate");
            }else{
                doauthenticate(successCallback, errorCallback);
            }
             },
             function (){
...
/*
 272371 / 2020 / Fiori Client IOS 13 - Scan Fingerprint / VAR Delivered Support
 When passcode screen pop up， Daymic tile may send request to server
 At that time  need to check if datavault is open then do the authenticate
*/

function doauthenticate(successCallback, errorCallback){
   utils.log("saml2 doauthenticate called");
